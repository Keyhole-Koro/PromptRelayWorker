...
const scoreSchema = z.object({
  playerImageUrl: z.string().url(),
  aiImageUrl: z.string().url(),
...
app.post("/v1/score", async (req, res) => {
  ...
  try {
    const result = await withTimeout(scoreImagesByUrl(parsed.data), config.REQUEST_TIMEOUT_MS, "request timeout");
    res.status(200).json(result);
  } catch (error) {
    if (error instanceof ScoreHttpError) {
