...
const RECENT_USED_SUBJECT_WINDOW = 5;
...
async function loadRecentUsedSubjects(): Promise<Set<string>> {
  const dirs = await listItemDirs(usedRoot());
  if (dirs.length === 0) {
    return new Set();
  }
  ...
  const metas = await Promise.all(
    dirs.map(async (dirPath) => {
      try {
        ...
        const meta = JSON.parse(raw) as PoolItemMeta;
        const subject = meta.genome?.subject;
        return {
          createdAt: meta.createdAt ?? "",
  ...
  const subjects = metas
    .filter((v): v is { createdAt: string; subject: string } => !!v && v.subject.length > 0)
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt))
    .slice(0, RECENT_USED_SUBJECT_WINDOW)
    .map((v) => v.subject);
...
export async function acquireAndMoveOneAvailableItem(aspectRatio: AspectRatio): Promise<{
  ...
  const recentSubjects = await loadRecentUsedSubjects();
  ...
  for (let attempt = 0; attempt < config.MAX_MOVE_RETRIES; attempt += 1) {
    ...
    for (const avoidRecentSubject of [true, false]) {
      for (const dirPath of shuffled) {
        ...
        const subject = claimed.meta.genome?.subject;
        if (avoidRecentSubject && typeof subject === "string" && recentSubjects.has(subject)) {
          await unlink(claimed.claimPath).catch(() => undefined);
          continue;
        }
